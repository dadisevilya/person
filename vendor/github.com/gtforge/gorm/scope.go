...
func (scope *Scope) inlineCondition(values ...interface{}) *Scope {
	if len(values) > 0 {
		// restrict only the first value
		// other values are treated as a query parameters
		// and will be passed via prepared statement
		scope.Err(restrictValue(values[0]))
...
// restrictValue restricts all strings which may include sql-injection
// this function is used for checking queries without prepared statements
func restrictValue(value interface{}) error {
	if str, ok := value.(string); ok {
		if !whitelistSQLInjectionRegexp.Match([]byte(str)) {
			return newValueRestrictedError(str)
		}
		if blacklistSQLInjectionRegexp.Match([]byte(str)) {
			return newValueRestrictedError(str)
		}
	}
	return nil
...
// whitelistSQLInjectionRegexp allows only prepare statement
// blacklistSQLInjectionRegexp matches expressions if they is comment in sql
var blacklistSQLInjectionRegexp = regexp.MustCompile(`(--.*)`)
var whitelistSQLInjectionRegexp = regexp.MustCompile(`^([\.A-Za-z0-9_-]+[\s]*(?i)([<>=]+|NOT LIKE|LIKE|NOT|IS|IS NULL|IS NOT NULL|((IN|NOT IN)[\s]*\([\s]*[?,\s]+))[\s]*([(]?[?)]+[)]?))[\s]*((?i)(OR|AND) ([\.A-Za-z0-9_-]+[\s]*(?i)([<>=]+|NOT LIKE|LIKE|NOT|IS|IS NULL|IS NOT NULL|((IN|NOT IN)[\s]*\([\s]*[?,\s]+))[\s]*([(]?[?)]+[)]?))[\s]*)*$`)

func newValueRestrictedError(value string) error {
	return ValueRestrictedError{
		value: value,
	}
}

type ValueRestrictedError struct {
	value string
}

func (err ValueRestrictedError) Error() string {
	return fmt.Sprintf("condition restricted: %s", err.value)
}
